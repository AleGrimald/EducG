...
public class LoginFrame extends JFrame {
    ...
    public LoginFrame() {
        setTitle("Educ G – Iniciar Sesión");
    ...
    private void buildUI() {
        ...
        // ── Panel Izquierdo: Información del Proyecto (30% del ancho) ────────
        ...
        // ── Panel Derecho: Formulario de Login (70% del ancho) ──────────────
    ...
    private JPanel createLeftPanel() {
        ...
        java.io.File imageFile = new java.io.File("assets/login-panel-izquierdo.png");

        if (imageFile.exists()) {
            try {
                ImageIcon imageIcon = new ImageIcon("assets/login-panel-izquierdo.png");
    ...
    private JPanel createRightPanel() {
        ...
        centerContainer.setMaximumSize(new Dimension(420, 600));
        centerContainer.setPreferredSize(new Dimension(420, 600));
        ...
        addCardRow(card, cardTitle, cardGbc, 0, new Insets(0, 0, 30, 0));

        // Email
        addCardRow(card, createSectionLabel("Correo Electrónico"), cardGbc, 1, new Insets(0, 0, 8, 0));
        emailField = createStyledField();
        emailField.setToolTipText("ejemplo@dominio.com");
        addCardRow(card, emailField, cardGbc, 2, new Insets(0, 0, 20, 0));

        // Contraseña
        addCardRow(card, createSectionLabel("Contraseña"), cardGbc, 3, new Insets(0, 0, 8, 0));
        passwordField = createStyledPasswordField();
        addCardRow(card, passwordField, cardGbc, 4, new Insets(0, 0, 12, 0));
        ...
        addCardRow(card, showPasswordCheck, cardGbc, 5, new Insets(0, 0, 28, 0));

        // Botón iniciar sesión
        JButton loginBtn = createLoginButton("Iniciar Sesión");
        loginBtn.addActionListener(e -> handleLogin());
        addCardRow(card, loginBtn, cardGbc, 6, new Insets(0, 0, 16, 0));

        // Divisor
        JSeparator sep = new JSeparator();
        sep.setForeground(new Color(220, 225, 230));
        addCardRow(card, sep, cardGbc, 7, new Insets(16, 0, 16, 0));

        // Texto sin cuenta
        JLabel noAccountLbl = new JLabel("¿No tenés cuenta?");
        noAccountLbl.setFont(new Font("Segoe UI", Font.PLAIN, 13));
        noAccountLbl.setForeground(new Color(100, 120, 140));
        noAccountLbl.setHorizontalAlignment(SwingConstants.CENTER);
        addCardRow(card, noAccountLbl, cardGbc, 8, new Insets(0, 0, 12, 0));

        // Botón registrarse
        JButton registerBtn = createRegisterButton("Crear Nueva Cuenta");
        registerBtn.addActionListener(e -> openRegister());
        addCardRow(card, registerBtn, cardGbc, 9, new Insets(0, 0, 0, 0));
    ...
    private void handleLogin() {
        ...
        try {
            if (AuthService.login(email, password)) {
                dispose();
                new CoursesFrame(email).setVisible(true);
            } else {
